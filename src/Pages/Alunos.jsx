...
function Alunos() { 
  ...
  // --- O useEffect volta ao original (com []) ---
  useEffect(() => {
    const fetchAlunos = async () => {
      try {
        ...
        console.error("ERRO DETALHADO (Alunos):", err);
      } finally {
        setLoading(false);
      }
    };

    fetchAlunos(); // ⬅️ Roda imediatamente
  }, []); // ⬅️ O array vazio funciona, pois o apiClient é criado acima

  // --- O RESTO DO CÓDIGO (handleSubmit, etc.) é IDÊNTICO ---
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const alunoData = { ...formData, cursos: [] };
    if (isEditing) {
      try {
        const response = await apiClient.put(`/alunos/${currentAlunoId}`, formData); 
        setAlunos(alunos.map(aluno => aluno.id === currentAlunoId ? response.data : aluno));
        clearForm();
        setError(null);
      } catch (err) { setError('Erro ao atualizar aluno.'); }
    } else {
      try {
        ...
        clearForm();
        setError(null);
      } catch (err) { setError('Erro ao cadastrar aluno.'); }
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Tem certeza?')) {
      try {
        ...
        setError(null);
      } catch (err) { setError('Erro ao deletar aluno.'); }
    }
  };
